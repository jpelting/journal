...
- `Entry` also carries a "Stoic daily reflection" set of fields (`morning_anxious_about`, `morning_within_control`, `morning_reserve_clause`, `evening_did_well`, `evening_where_falter`, `evening_could_improve`, `evening_gratitude_audit`), added directly to the morning/evening check-in forms per a user-supplied "Daily Stoic Journal" template — distinct from the Part 2 `stoic_prompt`/`stoic_response` guided journal. The week's practice itself isn't one of these stored fields: `StoicPractice` (`week_number` 1-52, unique) holds a curated 52-week curriculum, and `Entry.stoic_practice` is a property (like `introspection_prompt`) that calls `StoicPractice.for_date(self.date)` — deterministic by ISO week-of-year, capped at 52, so it's never stored or re-typed per entry.
